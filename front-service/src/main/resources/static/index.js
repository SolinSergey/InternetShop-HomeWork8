    ...
    function run($rootScope, $http, $localStorage) {
        if ($localStorage.marchMarketUser) {
            ...
            if (!$localStorage.marchMarketGuestCartId) {
                $http.get('http://localhost:7777/cart/api/v1/cart/generate_id')
        ...
        $http.post('http://localhost:7777/auth/authenticate', $scope.user)
            .then(function successCallback(response) {
                if (response.data.token) {
                    ...
                    $http.get('http://localhost:7777/cart/api/v1/cart/merge/'+$localStorage.marchMarketGuestCartId+'/'+$localStorage.marchMarketUser.username)
                        .then(function (response) {
                            $location.path('/');
                        });
                }
